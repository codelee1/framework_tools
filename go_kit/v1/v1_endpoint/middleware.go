	...
	return func(next endpoint.Endpoint) endpoint.Endpoint {
		return func(ctx context.Context, request interface{}) (response interface{}, err error) {
			defer func(begin time.Time) {
				logger.Debug(fmt.Sprint(ctx.Value(v1_service.ContextReqUUid)), zap.Any("调用 v1_endpoint LoggingMiddleware :"+method, "完成请求处理"), zap.Any("耗时毫秒", time.Since(begin).Milliseconds()))
			}(time.Now())
