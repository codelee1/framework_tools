...
func main() {
	...
	var (
		...
		server      = v3_service.NewService(logger, count, histogram)
		endpoints   = v3_endpoint.NewEndPointServer(server, logger, golangLimit)
		grpcServer  = v3_transport.NewGRPCServer(endpoints, logger)
		httpHandler = v3_transport.NewHttpServer(endpoints, logger)
	)
	var (
