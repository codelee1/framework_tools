...
import (
	...
	"framework_tools/go_kit/v3/v3_transport"
...
func TestHttpClient(t *testing.T) {
	...
	svc, err := v3_transport.NewHttpClient("127.0.0.1:8081", logger)
	if err != nil {
		t.Error(err)
		return
	}
	...
	ack := loginAck.(*v3_endpoint.LoginResp)
	...
	userAck := userInfoAck.(*v3_endpoint.UserInfoResp)
...
func TestGRPCClient(t *testing.T) {
	...
	var (
		endpoints v3_endpoint.Set
		kitlog    = log.NewNopLogger()
	)
	{
		loginFactory := factoryForGrpcService(v3_endpoint.MakeLoginClientEndPoint)
	...
	{
		userInfoFactory := factoryForGrpcService(v3_endpoint.MakeGetUserInfoClientEndPoint)
...
func factoryForGrpcService(makeEndpoint func(v3_service.Service) endpoint.Endpoint) sd.Factory {
	return func(instance string) (endpoint.Endpoint, io.Closer, error) {
		...
		//defer conn.Close()
		svc, err := v3_transport.NewGRPCClient(conn, logger)
		if err != nil {
			logger.Error("err:", zap.Error(err))
			return nil, nil, err
		}
