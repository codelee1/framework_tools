...
func (s baseServer) Login(ctx context.Context, account, password string) (resp interface{}, err error) {
	...
	resp = token
	s.logger.Debug(fmt.Sprint(ctx.Value(ContextReqUUid)), zap.Any("调用 v4_service Service", "Login 处理请求"), zap.Any("处理返回值", resp))
	return
}

func (s baseServer) UserInfo(ctx context.Context, token string) (resp interface{}, err error) {
	s.logger.Debug(fmt.Sprint(ctx.Value(ContextReqUUid)), zap.Any("调用 v4_service Service", "UserInfo 处理请求"))
	if token == "" {
	...
	s.logger.Debug(fmt.Sprint(ctx.Value(ContextReqUUid)), zap.Any("调用 v4_service Service", "UserInfo 处理请求"), zap.Any("处理返回值", resp))
	return
}
