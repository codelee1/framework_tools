...
func main() {
	fs := flag.NewFlagSet("v4", flag.ExitOnError)
	var (
	...
	var (
		...
		server      = v4_service.NewService(logger, count, histogram)
		endpoints   = v4_endpoint.NewEndPointServer(server, logger, golangLimit)
		grpcServer  = v4_transport.NewGRPCServer(endpoints, logger)
		httpHandler = v4_transport.NewHttpServer(endpoints, logger)
	)
	var (
