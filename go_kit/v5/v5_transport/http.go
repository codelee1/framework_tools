...
func NewHttpServer(endpoint v5_endpoint.Set, log *zap.Logger) http.Handler {
	options := []httptransport.ServerOption{
		httptransport.ServerBefore(func(ctx context.Context, request *http.Request) context.Context {
			ctx = context.WithValue(ctx, v5_service.ContextReqUUid, request.Header.Get(v5_service.ContextReqUUid))
...
func NewHttpClient(addr string, log *zap.Logger) (v5_service.Service, error) {
	if !strings.HasPrefix(addr, "http") {
		addr = "http://" + addr
	}
	u, err := url.Parse(addr)
	if err != nil {
		return v5_endpoint.Set{}, err
	}
	options := []httptransport.ClientOption{
		httptransport.ClientBefore(func(ctx context.Context, r *http.Request) context.Context {
			...
			r.Header.Set(v5_service.ContextReqUUid, UUID)
...
func decodeHttpUserInfoRequest(_ context.Context, r *http.Request) (interface{}, error) {
	var req v5_endpoint.UserInfoReq
	err := json.NewDecoder(r.Body).Decode(&req)
	if err != nil {
		return nil, err
	}
...
func decodeHttpLoginResponse(_ context.Context, r *http.Response) (response interface{}, err error) {
	...
	var resp v5_endpoint.LoginResp
...
func decodeHttpUserInfoResponse(_ context.Context, r *http.Response) (response interface{}, err error) {
	...
	var resp v5_endpoint.UserInfoResp
	err = json.NewDecoder(r.Body).Decode(&resp)
	return &resp, err
}
...
func encodeHTTPGenericRequest(_ context.Context, r *http.Request, request interface{}) error {
	...
	if req, ok := request.(v5_endpoint.UserInfoReq); ok {
